...
from evaluate_depth_acta import evaluate
from utils import *
...
class Trainer:
    def __init__(self, options):
        ...
        elif self.opt.encoder == "edgemono":
            self.models['encoder'] = networks.edgenext_base(self.opt.weights_init == "pretrained")

        self.models["encoder"].to(self.device)
        # monodepth setting
        if self.opt.lr_setting == "monodepth":
            self.parameters_to_train += list(self.models["encoder"].parameters())

        if self.opt.decoder == "depthdecoder" and self.opt.encoder != "resnet":
            self.models["depth"] = networks.DepthDecoder()
        elif self.opt.decoder == "depthdecoder" and self.opt.encoder == "resnet":
        ...
        elif self.opt.lr_setting == "actadepth":
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, 4, dataset_usage=DataSetUsage.TRAIN, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, 4, dataset_usage=DataSetUsage.VALIDATE, img_ext=img_ext)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            elif self.opt.encoder == "monovit":
                features = self.models["encoder"](inputs["color_aug", 0, 0])
            elif self.opt.encoder == "edgemono":
                _, features = self.models["encoder"](inputs["color_aug", 0, 0])
            else:
                features = self.models["encoder"](inputs["color_aug", 0, 0])
            # print(type(features))
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            # norm_disp.shape = torch.Size([1, 1, 192, 640])
            # color.shape = torch.Size([1, 3, 192, 640])
    ...
    def log_time(self, batch_idx, duration, loss):
        ...
        print(print_string.format(self.epoch + 1, self.model_optimizer.state_dict()['param_groups'][1]['lr'], #depth-lr
                                  self.model_optimizer.state_dict()['param_groups'][0]['lr'], #pose-lr
    ...
    def save_model(self):
        ...
        return save_folder
        # evaluate(self.opt.eval_mono=eva, save_folder)
